...
class SimpleVideoPlayer : public Component {
 ...
 protected:
  ...
  // Second full-size pixel buffer: a byte-for-byte copy of the canvas draw buf made once in
  // setup() (no size computed here -- canvas_draw_buf_->data_size verbatim). decode_frame_()
  // writes decode_target_ (off-screen); present_frame_() points canvas_draw_buf_->data at it
  // (pointer swap only, as youkorr's lvgl_camera_display does) so LVGL never reads a buffer a
  // decode is writing.
