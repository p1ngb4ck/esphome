...
static const uint16_t CACHE_BUF_ALIGN = 1024;
static const uint16_t EOI = 0xd9ff;
...
static esp_err_t get_video_size(uint32_t *width, uint32_t *height) {
  ...
  ESP_LOGI(TAG, "header parsed, width is %" PRId32 ", height is %" PRId32 ", size is %d", header.width, header.height,
           size);
...
lv_obj_t *SimpleVideoPlayer::lvgl_simple_player_create(lvgl_simple_player_cfg_t *params) {
  ...
  SimpleVideoPlayer::lvgl_simple_player_stop();
  return player_screen;
}

player_state_t SimpleVideoPlayer::lvgl_simple_player_get_state(void) { return player_ctx.state; }

void SimpleVideoPlayer::lvgl_simple_player_hide_controls(bool hide) {
...
void SimpleVideoPlayer::lvgl_simple_player_change_file(const char *video_file) {
  if (!player_ctx.is_init) {
    ESP_LOGW(TAG, "Not init");
    return;
  }
...
void SimpleVideoPlayer::lvgl_simple_player_play(void) {
  if (!player_ctx.is_init) {
    ESP_LOGW(TAG, "Not init");
    return;
  }
...
void SimpleVideoPlayer::lvgl_simple_player_pause(void) {
  if (!player_ctx.is_init) {
    ESP_LOGW(TAG, "Not init");
    return;
  }
...
void SimpleVideoPlayer::lvgl_simple_player_resume(void) {
  if (!player_ctx.is_init) {
    ESP_LOGW(TAG, "Not init");
    return;
  }
...
void SimpleVideoPlayer::lvgl_simple_player_stop(void) {
  if (!player_ctx.is_init) {
    ESP_LOGW(TAG, "Not init");
    return;
  }
  ...
  player_ctx.state = PLAYER_STATE_STOPPED;
    tx.img_stop, LV_OBJ_FLAG_HIDDEN);
}

void SimpleVideoPlayer::lvgl_simple_player_repeat(bool repeat) {
  if (!player_ctx.is_init) {
    ESP_LOGW(TAG, "Not init");
    return;
  }
...
esp_err_t SimpleVideoPlayer::lvgl_simple_player_del(void) {
  if (!player_ctx.is_init) {
    ESP_LOGW(TAG, "Not init");
    return ESP_OK;
  }
...
esp_err_t SimpleVideoPlayer::lvgl_simple_player_wait_task_stop(int timeout_ms) {
  if (!player_ctx.is_init) {
    ESP_LOGW(TAG, "Not init");
    return ESP_OK;
  }
