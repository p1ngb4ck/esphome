...
// Bytes requested per worker read_chunk() call. The stream API imposes no ceiling; this is only
// how much of one asset moves per round trip through the worker.
...
void FileExplorerAssets::loop() {
  ...
  for (size_t i = 0; i < this->asset_count_; i++) {
    if (this->assets_[i].psram == nullptr) {
      this->start_load_(i);
      return;  // one asset in flight at a time
    }
  }
#endif
...
void FileExplorerAssets::start_load_(size_t index) {
  ...
  // No stat and no direct read: the file is pulled in entirely through the worker's async stream
  // API (begin_read/read_chunk/end_read). The worker runs it on its task for task-safe media and
  // loop-sliced otherwise, serialized against everything else on the storage, and continues until
  // read_chunk() reports EOF -- into an EXTERNAL (PSRAM) buffer that grows as chunks arrive.
  this->loading_ = index;
...
void FileExplorerAssets::issue_read_() {
  // Grow the PSRAM buffer (doubling) so it can hold one more chunk, then ask the worker for it.
...
void FileExplorerAssets::abandon_load_(const char *reason, storage::StorageError err) {
  ...
  if (this->stream_open_) {
    // Release the pool slot even after a failed chunk, as end_read()'s contract demands.
