...
/**
 * @brief SPI MRAM storage device (Everspin MR25H series)
 *
 * SPI Magnetoresistive RAM with:
 * - Instant writes (no delays)
 * - Unlimited write cycles (magnetic storage)
 * - No erase needed
...
class SPIMRAM : public BinaryStorage,
 ...
 public:
  ...
  /**
   * @brief Set MRAM model/type
   *
   * @param model Model string (e.g., "MR25H256", "MR25H40")
   */
  ...
  const char *get_device_type() const override { return "spi_mram"; }
  ...
  // MRAM-specific features (no delays, no erase needed)
   ...
   *
   * Optimized for MRAM's fast writes
   *
 ...
 protected:
  ...
  std::string model_{"SPI_MRAM"};
  uint32_t capacity_{0};
  uint8_t addressing_bits_{16};  // 16 or 24

  //========================================================================
  // SPI MRAM Commands (standard SPI EEPROM compatible)
  //========================================================================
  ...
  static constexpr uint8_t CMD_RDID = 0x9F;   // Read device ID (JEDEC ID)
