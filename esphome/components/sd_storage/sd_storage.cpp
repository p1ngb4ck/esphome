...
bool SdMmc::mount_card() {
  ...
  esp_err_t pre_init_ret = sdmmc_host_init_slot(host.slot, &slot_config);
  if (pre_init_ret != ESP_OK) {
    ESP_LOGE(TAG, "Failed to init SDMMC slot %d: %s", this->slot_, esp_err_to_name(pre_init_ret));
    return false;
  }
#endif
