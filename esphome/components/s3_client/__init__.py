...
def _validate_tls(config):
    ...
    # entry (embedded or storage). A ca without tls is a configuration error.
    if not config.get(CONF_TLS, True) and CONF_CA in config:
...
async def to_code(config):
    for share in config:
        ...
        if share[CONF_TLS]:
            ...
            # Optional: an explicit cert_store entry. Absent -> C++ default (empty) -> the
            # built-in bundle is used by cert_store.
            if CONF_CA in share:
