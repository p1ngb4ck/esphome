...
void IDFUARTComponent::write_array(const uint8_t *data, size_t len) {
  ...
  for (size_t i = 0; i < len; i++) {
    this->debug_callback_.call(UART_DIRECTION_TX, data[i], StringRef(this->debug_prefix_.c_str());
  }
#endif
}
...
#ifdef USE_UART_DEBUGGER
  for (size_t i = 0; i < len; i++) {
    this->debug_callback_.call(UART_DIRECTION_RX, data[i], StringRef(this->debug_prefix_.c_str()));
  }
#endif
