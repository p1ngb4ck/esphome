...
def _final_validate(config: ConfigType) -> ConfigType:
    ...
    if CONF_API not in full:
        # No api: no network retrieval channel on this branch (the upstream
        # PR's GetYaml messages are not vendored) — nothing to protect.
        return config
...
# ---- Fork extension: export the embedded YAML to storage ----
CONF_PATH = "path"
CONF_RAW = "raw"

ExportToStorageAction = store_yaml_ns.class_("ExportToStorageAction", automation.Action)
...
EXPORT_ACTION_SCHEMA = cv.All(
    ...
    cv.Schema(
    {
        ...
        # raw=False: target DIRECTORY the original file tree is recreated in.
        # raw=True: target FILE the compressed .ehy1.zst envelope is written to.
...
@automation.register_action(
    "store_yaml.export_to_storage",
    ExportToStorageAction,
    EXPORT_ACTION_SCHEMA,
    synchronous=True,
)
async def store_yaml_export_to_code(config, action_id, template_arg, args):
    ...
    # Pulls in the export implementation and the vendored zstd decoder.
    cg.add_define("USE_STORE_YAML_EXPORT")
    return var
