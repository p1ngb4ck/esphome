...
3. **Component names**: ~~Naming decisions~~

   **Decided**: Consistent naming pattern:
   - `storage` - registry/interface
   - `binary_storage` - FRAM, EEPROM, Flash
   - `sd_storage` - SD/MMC cards (renamed from `sd_mmc_card`)
   - `usb_storage` - USB mass storage (renamed from `usb_msc_host`)
   - `http_file_browser` - web-based file browser (renamed from `http_file_server`)
   - Future: `network_storage` (NFS, SMB, etc.)

### 7.3 Libraries

4. **LittleFS library**: ~~Which approach for custom block device support?~~

   **Decided**: Fork `esp_littlefs` that combines:
   - Current `esp_littlefs` partition-based functionality
   - Raw `lfs.h` access (from upstream littlefs, tests removed)

   Implementation:
   - Add `#ifdef USE_LITTLEFS_CUSTOM_BLOCK_DEVICE` guard around raw `lfs.h` code
   - Define `USE_LITTLEFS_CUSTOM_BLOCK_DEVICE` only when `binary_storage` uses LittleFS mode
   - This ensures **zero additional binary size** when not using external block devices
   - Fork repo: `esphome/esp_littlefs` or similar
